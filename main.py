...
@app.post("/sa_removeUser", response_class=HTMLResponse)
...
@app.post("/sa_setUser", response_class=HTMLResponse)
...
@app.get("/sa_assignUserClass", response_class=HTMLResponse)
...
@app.post("/set_userClass", response_class=HTMLResponse)
async def set_userClass(
    ...
    try:
        ...
        db.commit()
        return RedirectResponse(url="/sa_assignUserClass", status_code=303)
    except Exception as e:
        print(f"An error occurred: {e}")
        db.rollback()
        request.session["error"] = "Something Went Wrong on Server !!!"
...
@app.get("/sa_AssignTeacherListings", response_class=HTMLResponse)
...
@app.get("/sa_AssignTeacher", response_class=HTMLResponse)
...
@app.post("/assignclass-sub", response_class=HTMLResponse)
    ...
    try:
       ...
       for data in data_list:
            ...
            if already_exist:
                ...
                return RedirectResponse(url=f"/sa_AssignTeacher?user_id={user_id}&user_name={user_name}", status_code=303)
       ...
       return RedirectResponse(url="/sa_AssignTeacherListings", status_code=303)

...
async def superadmin_root(request: Request, db: Session = Depends(get_db), is_auth= Depends(auth_required)
...
async def delete_user(request: Request, user_id: int = Form(...), db: Session = Depends(get_db), 
                      is_auth= Depends(auth_required), is_allowed= Depends(adminAllowed)):
    try:
...
async def assignuUserClass(request: Request, db: Session = Depends(get_db), is_auth= Depends(auth_required),
                           is_allowed= Depends(adminAllowed)):
    try:
...
async def AssignTeacherListings(request: Request, db: Session = Depends(get_db),is_auth= Depends(auth_required)):
    try:
        pending_class_users = db.query(User.user_id, User.full_name, User.email,
...
async def AssignTeacher(request: Request, user_id: str  , user_name: str , db: Session = Depends(get_db), is_auth= Depends(auth_required)):
    try:
